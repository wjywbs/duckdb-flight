...
def test_flight_sql_prepared_roundtrip(shell):
    port = find_free_port()
    client_binary = get_flight_client_binary(shell)
    proc = _start_flight_shell(shell, '-flight-sql', str(port))
...
def test_flight_sql_transaction_roundtrip(shell):
    port = find_free_port()
    client_binary = get_flight_client_binary(shell)
    proc = _start_flight_shell(shell, '-flight-sql', str(port))
    ...
    try:
        wait_for_server_ready(client_binary, port, timeout=20)
        client_result = run_flight_client(client_binary, port, 'transaction', timeout=90)
        assert (
            client_result.returncode == 0
        ), f"Transaction client failed with stdout='{client_result.stdout}' stderr='{client_result.stderr}'"
    finally:
        if proc.poll() is None:
            proc.send_signal(signal.SIGINT)
...
def test_flight_sql_cancel_roundtrip(shell):
    port = find_free_port()
    client_binary = get_flight_client_binary(shell)
    proc = _start_flight_shell(shell, '-flight-sql', str(port))
    ...
    try:
        wait_for_server_ready(client_binary, port, timeout=20)
        client_result = run_flight_client(client_binary, port, 'cancel', timeout=90)
        assert (
            client_result.returncode == 0
        ), f"Cancel client failed with stdout='{client_result.stdout}' stderr='{client_result.stderr}'"
    finally:
        if proc.poll() is None:
            proc.send_signal(signal.SIGINT)
